...
BUILD=build
PTAU_POWER=12
PTAU_FINAL="$BUILD/pot${PTAU_POWER}_final.ptau"
...
if [ ! -f "$PTAU_FINAL" ]; then
  npx --yes snarkjs powersoftau new bn128 "$PTAU_POWER" "$BUILD/pot${PTAU_POWER}_0000.ptau" -v
  npx --yes snarkjs powersoftau contribute "$BUILD/pot${PTAU_POWER}_0000.ptau" "$BUILD/pot${PTAU_POWER}_0001.ptau" \
    --name="Sharibo phase1 contribution" -v -e="$(head -c 64 /dev/urandom | base64)"
  npx --yes snarkjs powersoftau prepare phase2 "$BUILD/pot${PTAU_POWER}_0001.ptau" "$PTAU_FINAL" -v
fi
