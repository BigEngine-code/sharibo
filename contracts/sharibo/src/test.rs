...
fn create_circle_emits_created_event() {
    ...
    let event = events
        .iter()
        .find(|e| {
            e.topics().get(0).unwrap() == symbol_short!("circle")
                && e.topics().get(1).unwrap() == symbol_short!("created")
        })
        .unwrap();

    let topics = event.topics();
    assert_eq!(topics.get(0).unwrap(), symbol_short!("circle"));
...
fn fund_emits_funded_event() {
    ...
    let event = events
        ...
        .find(|e| {
            e.topics().get(0).unwrap() == symbol_short!("circle")
                && e.topics().get(1).unwrap() == symbol_short!("funded")
        })
        .unwrap();
    ...
    let (event_from, new_pot, target): (Address, i128, i128) =
        event.data().try_into_val(&s.env).unwrap();
...
fn claim_emits_claimed_event() {
    ...
    let event = events
        ...
        .find(|e| {
            e.topics().get(0).unwrap() == symbol_short!("circle")
                && e.topics().get(1).unwrap() == symbol_short!("claimed")
        })
        .unwrap();
...
fn cancel_circle_emits_cancelled_event() {
    ...
    let event = events
        ...
        .find(|e| {
            e.topics().get(0).unwrap() == symbol_short!("circle")
                && e.topics().get(1).unwrap() == symbol_short!("cancelled")
        })
        .unwrap();
